...
class Vice:
    ...
    def generate_explanation(self, index):
        ...
        self.__instance_explanation(
            sample_row,
            index,
            self.X_pos_array,
            self.bins_centred,
            self.no_bins,
            [],  # monotonicity_arr, replace with actual monotonicity array if available
            self.col_ranges
        )
        ...
        if direction == -1:
            ...
            if percent_up >= percent_down:
                if improve:
                    ...
                    pass  # TODO: Add logic for non-special case
                    c_current_bins[feat_idx] -= 1
    ...
    def __find_anchors(self, sample, no_val):
        ...
        avg_list, std_list = self.__evaluate_data_set()
